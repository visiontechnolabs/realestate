...
public function get_sites()
{
    ...
    // Fetch active sites
    $this->db->select('s.id, s.name, s.location, s.area, s.isActive,s.site_map, s.created_at');
...
public function search_sites()
{
    ...
    $this->db->select('s.id, s.name, s.location, s.area, s.isActive, s.created_at');
    $this->db->from('sites s');
    $this->db->join('site_assignments sa', 'sa.site_id = s.id', 'inner');
    $this->db->where('sa.user_id', $user_id);
    $this->db->where('s.admin_id', $admin_id);
...
public function get_plots($site_id = null)
{
    ...
    // 4️⃣ Fetch plots for this site (assigned to user)
    ...
    // 6️⃣ Build dynamic plot counts for this site
    $site = null;
    if ($site_row) {
...
public function plot_details($plot_id = null)
{
    ...
    // ----------------- 5. If Plot SOLD → Fetch Buyer Info -------------
...
public function add_expenses()
{
    ...
    // ✅ 2. Get admin_id of logged-in user
    $user = $this->db->select('admin_id')->where('id', $user_id)->get('users')->row();
    if (!$user || !$user->admin_id) {
        return $this->output
            ->set_status_header(400)
            ->set_output(json_encode([
                'status'  => false,
                'code'    => 400,
                'message' => 'Something went wrong. Admin not found for this user.',
                'data'    => null
            ]));
    }
    ...
    $required_fields = ['site_id', 'description', 'date', 'amount'];
    ...
    $site_id = (int) $input_data['site_id'];

    // ✅ 5. Check if this site is assigned to this user (like in get_sites)
    $this->db->select('s.id');
    $this->db->from('sites s');
    $this->db->join('site_assignments sa', 'sa.site_id = s.id', 'inner');
    $this->db->where('sa.user_id', $user_id);
    ...
    // ✅ 7. Insert expense data
    $insert = $this->db->insert('expenses', $expense_data);
...
$filter = $filter_input ? strtolower($filter_input) : null;
    $site_id = $this->input->get('site_id');

    // ⭐ Month filter (default → current month)
    $month_input = strtolower($this->input->get('month'));
    $month_number = null;
    ...
    // ✅ 5. Base query
    $this->db->select('id, admin_id, site_id, description, date, amount, status, isActive, created_at');
    ...
    foreach ($expenses as $e) {
        ...
        if ($e->status === 'approve') $approved_count++;
    }
